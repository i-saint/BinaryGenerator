#include "../BinaryGenerator.h"

void Generate_SayHello_COFF_x86()
{
    bg::Context ctx;

    // void SayHello() { MessageBoxA("hello!"); }
    bg::Section *text = ctx.createSection(".text", bg::SectionType_Text);
    const char code[] = "";
    text->addExternalSymbol(code, sizeof(code), "_SayHello");

    // option: dllexport SayHello()
    bg::Section *directive = ctx.createSection(".drectve", bg::SectionType_Info);
    const char exports[] = "/EXPORT:_SayHello ";
    directive->addStaticSymbol(exports, sizeof(exports), ".drectve");

    ctx.writeCOFFx86("SayHello_x86.obj");
}

void Generate_PutMessage_COFF_x64()
{
    bg::Context ctx;

    // void PutMessage() { puts("hello!"); }
    bg::Section *text = ctx.createSection(".text", bg::SectionType_Text);
    const char message[] = "hello!";
    bg::Symbol sym_str = text->addStaticSymbol(message, sizeof(message), "$0");

    const char code[] = "\x48\x8D\x0D\x00\x00\x00\x00\xE9\x00\x00\x00\x00";
    bg::Symbol sym_func = text->addExternalSymbol(code, sizeof(code), "PutMessage");

    text->addRelocation(sym_func.rva + 3, "$0", bg::RelocationType_REL32);
    text->addRelocation(sym_func.rva + 8, "puts", bg::RelocationType_REL32);

    ctx.writeCOFFx64("PutMessage_x64.obj");
}


int main(int argc, char *argv[])
{
    Generate_SayHello_COFF_x86();
    Generate_PutMessage_COFF_x64();
}
