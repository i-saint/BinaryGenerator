#include "../BinaryGenerator.h"

void Generate_SayHello_x86()
{
    bg::Context ctx;

    {
        // void SayHello() { MessageBoxA("hello!"); }
        bg::Section *text = ctx.createSection(".text1", bg::SectionType_Text);
        const char bin[] = "";
        text->addSymbol(bin, sizeof(bin), "_SayHello");
    }

    {
        // option: dllexport to SayHello()
        bg::Section *directive = ctx.createSection(".drectve", bg::SectionType_Info);
        const char dtv[] = "/EXPORT:_SayHello ";
        directive->addSymbol(dtv, sizeof(dtv), ".drectve");
    }

    ctx.writeCOFFx86("SayHello_x86.obj");
}


int main(int argc, char *argv[])
{
    Generate_SayHello_x86();
}
